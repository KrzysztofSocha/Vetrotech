...
namespace Vetrotech.Tests
{
    public abstract class VetrotechTestBase : AbpIntegratedTestBase<VetrotechTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected VetrotechTestBase()
        {
        ...
        protected void UsingDbContext(Action<VetrotechDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<VetrotechDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<VetrotechDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<VetrotechDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<VetrotechDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<VetrotechDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<VetrotechDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<VetrotechDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<VetrotechDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<VetrotechDbContext, Task<T>> func)
        {
            T result;
